...
const { openAi, generateImage } = require("../../lib/openai");
...
router.put("/:item", auth.required, async function (req, res, next) {
    ...
    if (req.item.seller._id.toString() === req.payload.id.toString()) {
        ...
        if (
            typeof req.body.item.image === "string" &&
            req.body.item.image.length > 0
        ) {
            req.item.image = req.body.item.image;
        } else {
            req.item.image = await generateImage(req.body.item.title);
        }
